...
describe("Direction1155C - End to End Flow", function () {
  ...
  it("reverts executeMint if no approvals have been granted", async () => {
    const id = 2001;
    const fees = { notaryFee: 0, managerFee: 0, tax: 0 };
    await c.requestMint(owner.address, id, 5, fees, "", ethers.ZeroHash, "");
    const reqId = (await c.mintRequestId()) - 1n;
    await expect(c.executeMint(reqId)).to.be.revertedWith("need 2-of-2");
  });

  it("reverts executeMint when only notary approval is present", async () => {
    const id = 2002;
    const fees = { notaryFee: 0, managerFee: 0, tax: 0 };
    await c.requestMint(owner.address, id, 5, fees, "", ethers.ZeroHash, "");
    const reqId = (await c.mintRequestId()) - 1n;
  ...
  it("reverts executeMint when only manager approval is present", async () => {
    const id = 2003;
    const fees = { notaryFee: 0, managerFee: 0, tax: 0 };
    await c.requestMint(owner.address, id, 5, fees, "", ethers.ZeroHash, "");
    const reqId = (await c.mintRequestId()) - 1n;
  ...
  it("restricts document and URI setters to their respective roles", async () => {
    const NOTARY_ROLE = await c.NOTARY_ROLE();
    const MANAGER_ROLE = await c.MANAGER_ROLE();
    await expect(
      c.setDocument(4001, ethers.keccak256(ethers.toUtf8Bytes("fake")), "ipfs://doc")
    )
      .to.be.revertedWithCustomError(c, "AccessControlUnauthorizedAccount")
      .withArgs(admin.address, NOTARY_ROLE);

    await expect(
      c.setURI(4001, "ipfs://custom/{id}.json")
    )
      .to.be.revertedWithCustomError(c, "AccessControlUnauthorizedAccount")
      .withArgs(admin.address, MANAGER_ROLE);
  });
});
