...
describe("Direction1155C - KYC, Freeze & Pause", function () {
  ...
  it("prevents minting tokens to frozen accounts", async () => {
    const id = 8888;
    const fees = { notaryFee: 0, managerFee: 0, tax: 0 };
    ...
    await c.connect(notary).approveByNotary(reqId);
    await c.connect(manager).approveByManager(reqId);
    await c.connect(manager).setFreezeAccount(owner.address, true);
    await expect(c.executeMint(reqId)).to.be.revertedWith("frozen");
  });

  it("blocks frozen accounts from transferring tokens", async () => {
    const id = 777;
    await c.setKyc(bob.address, true);
    await c.connect(manager).setFreezeAccount(owner.address, true);
    await expect(
